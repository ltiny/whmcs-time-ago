...
function wikiDate($format, $time) {
	...
	foreach($values as $secs=>$point) {
		...
		if ($date_tooltip == true) {
			if ($persian_date == true) {
				if ($res >= 1)
...
function wikiPersianDate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'en')
{
    $T_sec = 0;
    ...
    for ($i = 0; $i < $sl; $i++) {
        ...
        switch ($sub) {
            ...
            case 'D':
                $out .= wikiPersianDate_words(array(
                    'kh' => $date[7]
                ), ' ');
                break;
            ...
            case 'r':
                $key = wikiPersianDate_words(array(
                    'rh' => $date[7],
                    'mm' => $j_m
                ));
        ...
        if ($m == '') {
            ...
            if ($s == '') {
                ...
                if ($jm == '') {
                    ...
                    $wikiPersianDate = explode('_', wikiPersianDate('Y_j', '', '', $timezone, 'en'));
                    if ($jd == '') {
                        list($gy, $gm, $gd) = jalali_to_gregorian($wikiPersianDate[0], $jm, $wikiPersianDate[1]);
                        return mktime($h, $m, $s, $gm);
                    } else {
                        if ($jy == '') {
                            list($gy, $gm, $gd) = jalali_to_gregorian($wikiPersianDate[0], $jm, $jd);
...
function jgetdate($timestamp = '', $none = '', $timezone = 'Asia/Tehran', $tn = 'en')
{
    ...
    $wikiPersianDate = explode('_', wikiPersianDate('F_G_i_j_l_n_s_w_Y_z', $ts, '', $timezone, $tn));
    return array(
        'seconds' => tr_num((int) tr_num($wikiPersianDate[6]), $tn),
        'minutes' => tr_num((int) tr_num($wikiPersianDate[2]), $tn),
        'hours' => $wikiPersianDate[1],
        'mday' => $wikiPersianDate[3],
        'wday' => $wikiPersianDate[7],
        'mon' => $wikiPersianDate[5],
        'year' => $wikiPersianDate[8],
        'yday' => $wikiPersianDate[9],
        'weekday' => $wikiPersianDate[4],
        'month' => $wikiPersianDate[0],
        0 => tr_num($ts, $tn)
    );
}
...
/*    F    */
function wikiPersianDate_words($array, $mod = '')
{
